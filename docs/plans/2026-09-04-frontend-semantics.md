...
- [ ] Документи сесії закомічені (задача 0 плану TypeScript).
...
- [ ] **Step 1:** тест: для кожного з семи файлів — жоден `v-for` не стоїть на `<p`; треклісти/лайнапи/списки зв'язків рендеряться як `<ol`/`<ul` з `<li`; у `playlist/[id].vue` немає `<ol>` → `<div>` → `<li>` (перевірка: після `<ol` перший дочірній тег — `<li`).
- [ ] **Step 2:** `release/[id].vue` — два треклісти і два списки зв'язків → `<ol class="list-none">` / `<ul class="list-none">` + `<li class="mb-2 …">`.
- [ ] **Step 3:** `track/[id].vue`, `artist/[id].vue`, `event/[id].vue`, `tracks.vue` — те саме.
- [ ] **Step 4:** `playlist/[id].vue` — реліз стає `<li>` з вкладеним `<ol>` треків.
- [ ] **Step 5:** `PagePlayer.vue` — рядки треків → `<ol>` + `<li>`; класи рядка на `<li>`.
- [ ] **Step 6:** гейт; commit `fix(a11y): render tracklists and related links as lists`.
...
- [ ] **Step 1:** тест: у шести файлах немає `:alt="` з `' Thumbnail'` / `' icon'` / `+ ' Icon'`; `<img` усередині цих компонентів має `alt=""`.
- [ ] **Step 2:** правки: `alt=""` на зображеннях у посиланнях із текстом; кнопки — `alt=""`, назва в `title`/тексті.
- [ ] **Step 3:** гейт; commit `fix(a11y): mark images inside labelled links as decorative`.
...
- [ ] **Step 1:** тест: кожен `<iframe` у `app/pages/**` має `title` або `:title`; жодне `:title` не конкатенує без пробілу (`+ 'YouTube`).
- [ ] **Step 2:** пробіл у шести YouTube-назвах; `title="SoundCloud player"` на iframe артиста; «Iframe» → «player».
- [ ] **Step 3:** гейт; commit `fix(a11y): give every embedded player a title`.
...
- [ ] **Step 1:** розширити наявну перевірку `CollectionStatus` трьома сторінками.
- [ ] **Step 2:** деструктуризувати `status`, `error`, `refresh` і вставити `<CollectionStatus>` за зразком `releases.vue`.
- [ ] **Step 3:** гейт; commit `fix(ui): show loading, empty and error states on the remaining lists`.
...
- [ ] **Step 1:** тест: кожен `<img` у чотирьох файлах має `height`/`:height`.
- [ ] **Step 2:** значення зі спеки (`OpenImage` 190/158, `RelativeItem` 24, портфоліо/друг 120×120).
- [ ] **Step 3:** гейт; commit `fix(perf): declare image dimensions on catalog media`.
...
- [ ] **Step 1:** тест: `input type="range"` у `AudioMixPlayer` має `aria-label` і клас `player-range`.
- [ ] **Step 2:** `aria-label="Seek"`, `class="player-range"`, `:style="{ '--progress': … }"` як у `PlayerSeek`; прибрати `accent-[#…]`.
- [ ] **Step 3:** гейт; commit `fix(a11y): name the mix seek slider and reuse the player range style`.
...
- [ ] **Step 1:** тест: `nuxt.config.ts` містить `color-scheme` meta; `tailwind.css` — `color-scheme: light` у `:root` і `color-scheme: dark` у `.dark`.
- [ ] **Step 2:** правки.
- [ ] **Step 3:** гейт; commit `fix(ui): declare color-scheme for both themes`.
...
- [ ] **Step 1:** тест: `@theme` містить `--color-moss` і `--color-moss-dark`; `#b5ccb5`/`#2a4030` не зустрічаються поза `@theme`; `HomepageAtmosphere` не містить `linear-gradient(` з літеральними кольорами.
- [ ] **Step 2:** токени + `--forest-tint-light/dark`; `bg-moss`/`fill-moss`/`dark:bg-moss-dark`; `html::after` і `.homepage-atmosphere::after` через змінні.
- [ ] **Step 3:** гейт; commit `refactor(css): move moss colors and forest tints into theme tokens`.
...
- [ ] **Step 1:** тест: у двох сторінках немає `<style`; `tailwind.css` містить `.BandcampIframe` і `.SoundcloudIframe` з `tracks-22`.
- [ ] **Step 2:** злити карти (надмножина), видалити `<style>` блоки.
- [ ] **Step 3:** гейт; commit `refactor(css): share the embedded player height map`.
...
- [ ] **Step 1:** тест: немає `Relative Releases`/`Relative Artists`, немає кириличної `Сredits`; `Hero.vue` має один `<h1` і він містить `Psychedelic Music Label`; `artist/[id].vue` рендерить секцію релізів під `v-if` з довжиною.
- [ ] **Step 2:** «Related …» ×3, `Credits`, `Coming soon` ×3.
- [ ] **Step 3:** `Hero.vue` — `<h1>` як обгортка обох рядків, внутрішні `span class="block"`.
- [ ] **Step 4:** `artist/[id].vue` — `artistReleases` computed, секція під `v-if="artistReleases.length"`.
- [ ] **Step 5:** гейт; commit `fix(ui): correct catalog copy, hero heading and empty artist sections`.
...
- [ ] **Step 1:** тест: у `LikeButton`, `PagePlayer`, `Header` рядки з `{{` не містять `opacity-50`/`opacity-60`/`opacity-[0.4]`; `Footer` без `text-white/50`; `artists/all.vue` без `hover:text-white/80`.
- [ ] **Step 2:** правки за спекою п. 5.
- [ ] **Step 3:** гейт; commit `fix(a11y): drop stacked opacity from text and lift footer contrast`.
...
- [ ] **Step 1:** `npm run test:e2e` один раз. Якщо снапшоти падають — відкрити diff-зображення; оновлювати (`test:e2e:update`) лише якщо змінені області — підпис хедера й текст футера. Інший diff — знайти задачу, що зрушила пікселі, і виправити.
- [ ] **Step 2:** AGENTS.md — нові інваріанти (треклісти `<ol>`, `alt=""` у посиланнях із текстом, без `opacity-*` на тексті в напівпрозорому батьку, `--color-moss`, `title` на кожному iframe); `Last reviewed: 2026-09-04`; під 250 рядків.
- [ ] **Step 3:** ініціатива → `Implemented`, roadmap синхронно, запис у `docs/completed.md`.
- [ ] **Step 4:** `npm run docs:check`; commit `docs: record the catalog semantics invariants and close the initiative`.
