...
У `ROADMAP.md`: видалити цілком секцію `### 2. Виправити /tracks data contract` (рядки 43–51, від заголовка до кінця списку «Рішення»), і додати в кінець розділу `## Закрито з попередніх аудитів` рядок:
...
```bash
git add tests/unit/catalogTracks.test.ts ROADMAP.md
...
```bash
git add "server/api/track/[id].get.ts" tests/unit/trackApiSimilar.test.ts AGENTS.md ROADMAP.md
