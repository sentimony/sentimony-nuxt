...
- [x] **Step 1: `[U]` Замінити NS у панелі Imena.ua на Cloudflare**

- [x] **Step 2: `[A]` Моніторити пропагацію**
...
- [x] **Step 3: `[A]` Перевірити, що нічого не зламалось**
...
- [x] **Step 5: `[A]` Заповнити колонку в аудиті й закомітити**
...
- [x] **Step 6: `[A]` Видалити мертву Netlify DNS zone**

Через Netlify API видалити зону `592b340ecf321c3f9608c3c4`. Тільки після того, як Cloudflare активний і заміри збіглися.

**Виконано (2026-08-10).** Перед видаленням усі 4 записи зони звірені з живим Cloudflare-DNS: `TXT google-site-verification` збігається байт у байт, `www` і `aquadeep` резолвляться, apex віддає `104.198.14.52` (зона тримала застарілий `75.2.60.5` — обидва Netlify LB). Зона стояла на `nsone.net` NS, тобто поза деревом делегування ще від часів Imena.ua. `DELETE` → 204; у Netlify не лишилось жодної зони `sentimony`. Сайт `aquadeep-jekyll` (`site_id` зони) не постраждав: `state: current`, SSL активний. Після видалення `web-debug` по проду — 20/20, `aquadeep` і `www` відповідають як раніше. Бекап зони збережено поза репозиторієм.

- [x] **Step 7: `[A]` Повернути TTL до 3600**

Через Cloudflare API або лишити `auto`.

**Рішення (2026-08-10).** TTL лишається **300** — свідомо, за рішенням власника. Поки фаза 3 не закрита остаточно, короткий TTL тримає вікно відкату в п'ять хвилин замість години; вартість — трохи більше запитів до Cloudflare, що на цьому обсязі не має значення. Підняти до 3600 варто тоді, коли доля міграції вирішена в будь-який бік.
