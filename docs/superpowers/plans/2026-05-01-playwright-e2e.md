...
**Передумова:** Плани 1-3 виконано (Vitest unit + server + component тести, 35 тестів зеленіють через `npm test`). `package.json` містить `test`/`test:watch`. `.env.stage` існує і має `NUXT_PUBLIC_SUPABASE_URL`, `NUXT_PUBLIC_SUPABASE_KEY`, `NUXT_SUPABASE_SECRET_KEY` (вказує на `supabase-stage` — план `2026-05-02-test-environments` виконано).
...
- `.env.stage` вказує на `supabase-stage` — окремий проєкт від prod. Тести ніколи не торкаються production-БД.
- У CI — ті самі stage-secrets через GitHub Secrets (Roadmap R1).
...
Run:
```bash
NUXT_PUBLIC_SUPABASE_URL=$(grep NUXT_PUBLIC_SUPABASE_URL .env.stage | cut -d= -f2-) \
NUXT_SUPABASE_SECRET_KEY=$(grep NUXT_SUPABASE_SECRET_KEY .env.stage | cut -d= -f2-) \
E2E_TEST_RELEASE_SLUG=$(grep E2E_TEST_RELEASE_SLUG .env.stage | cut -d= -f2-) \
...
- [ ] **Step 4: Перевірити, що секретні env-файли не закомічені**

Run: `git ls-files .env.stage .env.prod`
Expected: порожній вивід (обидва gitignored). Якщо хоч один трапився — критична помилка, видалити з історії окремо.
