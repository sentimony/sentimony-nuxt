...
<template>
  <div data-testid="site-header" class="sticky top-0 left-0 w-full z-20 border-b border-black/30 dark:border-white/30 bg-white/40 dark:bg-white/5 backdrop-blur-sm overflow-hidden">
    <div class="px-0">
      <div class="container max-w-7xl">
        <div class="relative flex justify-between items-center h-[75px] px-2">
          ...
          <NuxtLink
            to="/"
            class="w-[230px] transition-[background-color] ease-in-out duration-300 hover:bg-black/15 dark:hover:bg-white/30 h-[56px] flex items-center justify-center rounded-[2px]"
          ...
          <div class="hidden lg:flex gap-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            <NuxtLink
              ...
              :key="i.route"
              :to="i.route"
              class="transition-[background-color] ease-in-out duration-300 text-sm hover:bg-black/15 dark:hover:bg-white/30 px-4 h-9 inline-flex items-center justify-center gap-2 rounded-md"
          ...
          <div class="hidden xl:flex gap-2">
            ...
            >
              <a
                :href="i.url"
                class="flex items-center justify-center transition-[background-color] ease-in-out duration-300 hover:bg-black/15 dark:hover:bg-white/30 size-9 rounded-md relative z-30"
          ...
          <NuxtLink
            v-if="user"
            to="/profile"
            class="transition-[background-color] ease-in-out duration-300 flex items-center justify-center hover:bg-black/15 dark:hover:bg-white/30 size-9 rounded-md"
          ...
          <NuxtLink
            v-else
            to="/signin"
            class="transition-colors ease-in-out duration-300 inline-flex items-center justify-center gap-2 border border-black/20 dark:border-white/30 hover:bg-black/10 dark:hover:bg-white/20 hover:border-black/30 dark:hover:border-white/50 px-4 h-9 rounded-md text-sm"
            :class="isNavActive('/signin') ? 'bg-black/10 dark:bg-white/20 border-black/30 dark:border-white/50' : ''"
            v-wave
          >
