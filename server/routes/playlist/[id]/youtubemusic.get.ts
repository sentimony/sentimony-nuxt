...
export default defineEventHandler(async (event) => {
  ...
  const youtubeMusic = playlist?.links?.youtube_music as string | undefined
  if (!youtubeMusic) {
    throw createError({ statusCode: 404, statusMessage: 'YouTube Music link not found' })
  }

  // Permanent redirect for SEO
  return sendRedirect(event, youtubeMusic, 301)
})
