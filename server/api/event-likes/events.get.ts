...
export default defineEventHandler(async (event) => {
  ...
  const [{ count }, { data: likes }] = await Promise.all([
    ...
    admin.from('event_likes').select('event_slug').eq('user_id', userId).order('created_at', { ascending: false }).range(from, to),
  ])
  ...
  const sorted = slugs.map(slug => data?.find(e => e.slug === slug)).filter(Boolean)
