...
export default defineEventHandler(async (event) => {
  ...
  const [{ count }, { data: likes }] = await Promise.all([
    ...
    admin.from('track_likes').select('track_slug').eq('user_id', userId).order('created_at', { ascending: false }).range(from, to),
  ])
  ...
  const sorted = slugs.map(slug => data?.find(t => t.slug === slug)).filter(Boolean)
