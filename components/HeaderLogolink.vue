<template>
  <nuxt-link v-ripple :to="logolink.url" class="header-logolink" active-class="is-selected" exact>
    ...
    />
    <div class="header-logolink__text">
      <div class="header-logolink__title">{{ logolink.title }}</div>
      <div class="header-logolink__description">{{ logolink.description }}</div>
    </div>
  </nuxt-link>
</template>
    ...
    data() {
      return {
        logolink: {
          ...
          logoImg: 'https://content.sentimony.com/assets/img/svg-icons/sentimony-records-logo.svg',
          url: '/'
        }
      }
    }
