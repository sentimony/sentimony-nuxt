...
**Releases genre filter.** `/releases/{all,psytrance,psychill,ungrouped}` share `<GenreTabs>` (tab buttons with live per-genre counts) + `<ReleasesFiltered>`. The predicate is `matchesGenre(style, keyword)` (`app/utils/releaseGenres.ts`): case-insensitive `style.includes(keyword)`, except `ungrouped` = visible releases whose `style` contains **neither** `psytrance` **nor** `psychill` (`RELEASE_GENRES`). Per-genre counts on the tabs and the grid both filter through the same util.
