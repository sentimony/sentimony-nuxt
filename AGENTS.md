...
- **Buttons** live in `app/components/buttons/` with a dedicated `{ path: '~/components/buttons', pathPrefix: false }` entry in `nuxt.config` so they resolve as `<PrimaryButton>` / `<LikeButton>` (Nuxt prefixes nested component names by default → without it they'd be `<ButtonsPrimaryButton>`). Both wrap `ui/button`: `PrimaryButton` = link/icon wrapper (internal-vs-external `<a>`/`NuxtLink` detection, `glass` variant), `LikeButton` = accumulate/clap like button (`glass` for `size="lg"`, `soft` otherwise). `ui/button` is the single styling authority (`buttonVariants` cva: `outline`/`link`/`glass`/`soft`).
