...
npm run convert:yml     # regenerate sentimony-db-export.json from sentimony-db.yml (source of truth)
...
Якщо додається новий артист: додати запис у `server/data/sentimony-db.yml` (потім `npm run convert:yml`), призначити наступний вільний тризначний `category_id`, поставити запис у порядок за правилами вище, а коли з'явиться фото — додати відповідний рядок у `sentimony-images/src/data/artist-images.ts`.
