...
Current focused verification baseline: `npm run test:unit` -> 34 files / 146 tests; `npx nuxi typecheck` passes with local Supabase env warnings.
...
**Composables.** Each entity has `useXxx()` (wraps `useAsyncData` + `$fetch('/api/xxx')`) and `useXxxLikes()` built on the `createLikes(key, apiBase)` factory (`app/composables/createLikes.ts`). `useLikes()` (`app/composables/useLikes.ts`) is the release reference; artist/video/track/event/playlist variants follow it. A failed like reverts the optimistic update and fires `toast.error` (vue-sonner); `<Toaster>` is mounted once in `app/app.vue`. `toArray()` (`app/composables/toArray.ts`) normalises Firebase object-keyed and Supabase array responses into one array.

**Likes (anonymous + accumulate/clap).** Anyone can like every entity (release/artist/track/video/event/playlist) — **no auth guard, no `/signin` redirect**. Guests get an anonymous device-id: `createLikes` generates `crypto.randomUUID()` on first like and writes cookie `sentimony_anon_id` synchronously via `document.cookie` (`useCookie` flushes async, so the first `$fetch` would otherwise miss it). Server `getUserId()` (`server/utils/likes.ts`) returns `user.sub ?? user.id`, else the validated UUID from that cookie. **Model is accumulate, not toggle:** every click = +1 (clap), unlimited per user — there is **no unlike/DELETE** in the client flow. Storage is a `count int` column per `(user_id, slug)` row; increment goes through the atomic security-definer RPC `increment_like(p_table, p_slug_col, p_slug, p_user_id)` (whitelist tables/cols against SQL injection). Public counter = `SUM(count)` (not `COUNT(*)`) in `likeCounts.ts`; `/api/likes` returns `{slug, count}[]`. Client keeps `Math.max(local, server)` in `load()`/`setCount` so a reload never drops freshly-accumulated clicks while the cached catalog `like_count` lags. Migrations: `20260718_likes_accumulate_count.sql` (count col + RPC) and `20260718_likes_allow_anonymous.sql` (drops the 6 `*_likes_user_id_fkey` FKs on `auth.users` that blocked anon UUIDs — verify FKs via `pg_constraint`, **not** `information_schema`, which hides FKs whose ref table is in the `auth` schema).
